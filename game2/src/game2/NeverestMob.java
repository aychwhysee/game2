...
public class NeverestMob {
    ...
    public NeverestMob(int b_width, int b_height, int health) {
    ...
    public NeverestMob(Posn posn, int b_width, int b_height, int health) {
        this.posn = posn;
        this.b_width = b_width;
        this.b_height = b_height;
        this.health = health;
    ...
    public NeverestMob move() {
        ...
        if (randomDir == 1) {
            return new NeverestMob()
        }
        // randomly moving by itself
    }

    public NeverestMob react(NeverestPlayer player) {
    ...
    public boolean hitOnLeft(NeverestPlayer player) {
        // check left x posn
    ...
    public boolean hitOnRight(NeverestPlayer player) {
        // check right x posn
        return ((player.posn.x - player.width / 2 <= this.posn.x + this.width / 2)
                && (player.posn.y - player.height / 2 <= this.posn.y + this.height / 2)
                && (player.posn.y + player.height / 2 >= this.posn.y - this.height / 2));
    ...
    public boolean hitOnTop(NeverestPlayer player) {
        // check top y posn
    ...
    public boolean hitOnBot(NeverestPlayer player) {
        // check bottom y posn
        return ((player.posn.y - player.height / 2 >= this.posn.y + this.height / 2)
                && (player.posn.x - player.width / 2 <= this.posn.x + this.width / 2)
                && (player.posn.x + player.width / 2 >= this.posn.x - this.width / 2));
    ...
    public boolean hitAtAll(NeverestPlayer player) {
